...
enpm.prototype = {
	...
	installShrinkwrap: function(root, shrinkwrap) {
		...
		return Q.all(allVersions.map(function(pkg){
	...
	_linkPackage: function(root, target, depName, depVersion) {
		...
		if (fs.existsSync(linkName)) {
			if (fs.lstatSync(linkName).isSymbolicLink()) {
				...
				self.log.warn("Could not link " + depName + "@" + depVersion + " to " + path.relative(process.cwd(),linkName) + " file exists and is not a link");
				return;
			}
		}
